...
void
ins_node_t::vers_update_end(row_prebuilt_t*	prebuilt, bool history_row)
{
	...
	if (history_row) {
		set_tuple_col_8(row, table->vers_end, trx->id,
				vers_end_buf);
	} else /* ROW_INS_VERSIONED */ {
		set_tuple_col_8(row, table->vers_end, TRX_ID_MAX,
				vers_end_buf);
#ifndef DBUG_OFF
		...
		ut_ad(t->mysql_col_len == 8);
#endif
		set_tuple_col_8(row, table->vers_start, trx->id,
				vers_start_buf);
	}
	...
	col_no++) {
		...
		for (ulint i = 0; i < unsigned(v_col->num_base); i++) {
			...
			if (base_col->ind == table->vers_end) {
				innobase_get_computed_value(
					row, v_col, clust_index, &local_heap, table->heap, NULL, thd,
